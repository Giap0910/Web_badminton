...
| Large web monorepo (27K files) | 739,352 tokens | 15,049 tokens | **49.1x** |
...
- You work with **monorepos**, microservices, or complex multi-tier architectures
...
Java 17, JavaScript (React JSX), SQL (MySQL 8), CSS (Tailwind), HTML, XML/JSON.
...
### Prerequisites
- Supported local CLI or Node.js/Java runtime
...
| **Cross-boundary calls** (Frontend calling Backend REST API) | No direct static edges across HTTP protocol | Use API specification or mock contracts |
| **Stale graph** (without watch mode) | Queries outdated relationships | Always run `code-review-graph update` before tasks, or use watch mode |
| **Vite path aliases** (`@/components/...`) | May require path alias mapping | Configure Vite resolver paths |
...
|------|----------|------|------|----------|
| **code-review-graph** | Tree-sitter + SQLite | Fast, local AST indexing, no cloud deps | Static analysis only | General use, large codebases |
