...
**Execute these for frontend validation:**

| Task | Purpose | Usage |
|------|---------|-------|
| `npm run build` | Validate JSX compilation & Vite bundling | Run in `frontend/` directory |
| `npm run lint` | ESLint check for React & JSX best practices | Run in `frontend/` directory |
...
## 5. React 18 JSX Modern Form & Search Patterns

> [!IMPORTANT]
> For Web_badminton, use React 18 controlled/uncontrolled forms with React Router DOM for search and filter operations.

### Clean Search Form Example (Pure JSX)
```jsx
  ...
  return (
    <form onSubmit={handleSearch} className="relative flex items-center w-full">
      ...
      <input 
        ...
        placeholder="Tìm kiếm vợt, giày Yonex, Victor..." 
        className="w-full bg-slate-800/80 focus:bg-white text-white focus:text-slate-900 pl-10 pr-4 py-2 rounded-full border border-slate-700 focus:border-royal focus:outline-none transition-all"
      />
    </form>
  );
}
...
### Form Best Practices:
- **Use pure `.jsx`** with Lucide React icons.
- **Prevent page reloads** using `e.preventDefault()` and handle route transitions with `useNavigate()`.
- **Integrate smoothly** with Tailwind CSS design tokens (`#131B2E`, `#EF4444`, `#2563EB`).
